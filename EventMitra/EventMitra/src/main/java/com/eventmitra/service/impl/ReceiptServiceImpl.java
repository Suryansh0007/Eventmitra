...
public class ReceiptServiceImpl implements ReceiptService {
    ...
    public Receipt findById(Long id) {
        return receiptRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Receipt not found: " + id));
    }

    @Override
    public Receipt findByPayment(Long paymentId) {
        return receiptRepository.findByPaymentId(paymentId)
                .orElseThrow(() -> new ResourceNotFoundException("Receipt not found for payment: " + paymentId));
    }
}
