...
public class EventServiceImpl implements EventService {
    ...
    private Event map(Event event, EventRequest request) {
        var organizer = userRepository.findById(request.organizerId())
                .orElseThrow(() -> new ResourceNotFoundException("Organizer not found: " + request.organizerId()));
        ...
        event.setStatus(request.status() == null ? EventStatus.UPCOMING : request.status());
